...
<div id="printRepCont">
    <table class="rep-table" id="ExcelTable">
        <thead style="opacity: 0; visibility: hidden;">
            <tr role="row">
                <td><b>S. No.</b></td>
                    <th><b>Name of the Scheme / Programme and Budget Allocation (Rs. Lakhs)</b></th>
                    <th><b>Objectives</b></th>
                    <th><b>Indicators</b></th>
                    <th><b>Baseline ({{ date('Y', strtotime('-2 years')) }} - {{ date('Y', strtotime('-1 years')) }})</b></th>
                    <th><b>Targets ({{ date('Y', strtotime('-1 years')) }} - {{ date('y') }})</b></th>
                    <th><b>Achievement ({{ date('Y', strtotime('-1 years')) }} - {{ date('y') }})</b></th>
                    <th><b>Targets ({{ date('Y') }} - {{ date('y', strtotime('+1 years')) }})</b></th>
                    <th><b>Achievement ({{ date('Y') }} - {{ date('y', strtotime('+1 years')) }}) upto {{ $month.', '.date('Y') }}</b></th>
                    {{-- <th><b>Target({{ date('Y', strtotime('+1 years')) }} - {{ date('y', strtotime('+2 years')) }})</b></th> --}}
                    <th><b>Indicators</b></th>
                    <th><b>Baseline ({{ date('Y', strtotime('-2 years')) }} - {{ date('Y', strtotime('-1 years')) }})</b></th>
                    <th><b>Targets ({{ date('Y', strtotime('-1 years')) }} - {{ date('y') }})</b></th>
                    <th><b>Achievement ({{ date('Y', strtotime('-1 years')) }} - {{ date('y') }})</b></th>
            ...
            @if(count($schemes))
                ...
                @foreach($schemes as $key=>$value)
                    ...
                    <tr>
                        ...
                        <td class="@if($value['opb'] == '&nbsp;') td-no-bord @endif" style="{{ $color }}">@php echo htmlspecialchars_decode($value['opb']); @endphp</td>
                        
                        <td class="@if($value['targ_last'] == '&nbsp;') td-no-bord @endif" style="{{ $color }}">@php echo htmlspecialchars_decode($value['targ_last']); @endphp</td>
                        
                        <td class="@if($value['status_last'] == '&nbsp;') td-no-bord @endif" style="{{ $color }}">@php echo htmlspecialchars_decode($value['status_last']); @endphp</td>
                        ...
                        <td class="@if($value['ocb'] == '&nbsp;') td-no-bord @endif" style="{{ $color1 }}">@php echo htmlspecialchars_decode($value['ocb']); @endphp</td>
                        
                        <td class="@if($value['oc_targ_last'] == '&nbsp;') td-no-bord @endif" style="{{ $color1 }}">@php echo htmlspecialchars_decode($value['oc_targ_last']); @endphp</td>
                        
                        <td class="@if($value['oc_status_last'] == '&nbsp;') td-no-bord @endif" style="{{ $color1 }}">@php echo htmlspecialchars_decode($value['oc_status_last']); @endphp</td>
