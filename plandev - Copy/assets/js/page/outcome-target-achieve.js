$(document).ready(function(){
	...
	$.ajax({async: true,
		url: siteUrl + "/api/outcome_achievent_head_info",
		headers: {
	...
	$.ajax({async: true,
		url: siteUrl + "/api/outcome_achievent_list",
		headers: {
			'Accept': 'application/json',
			'Authorization': 'Bearer ' + localStorage.token
		},
	...
	$('#achievement-add').on('submit', function(e){
        ...
        if($.validatr.validateForm($('#achievement-add'))){
			...
			$.ajax({async: true,
				url: siteUrl + "/api/outcome_achievent_save",
				headers: {
					'Accept': 'application/json',
					'Authorization': 'Bearer ' + localStorage.token
				},
	...
	$(document).on('click', '.editAchieve', function(){
 	...
 	$('#editAchievementForm').on('submit', function(e){
        ...
        if($.validatr.validateForm($('#editAchievementForm'))){
			...
			$.ajax({async: true,
				url: siteUrl + "/api/outcome_achievent_edit",
				headers: {
					'Accept': 'application/json',
					'Authorization': 'Bearer ' + localStorage.token
				},
 	...
 	$(document).on('click', '.deleteOutcomeAchievement', function(){
	var id = $(this).attr('data-id');
	if(confirm("Are you sure to delete?")){
		
		$.ajax({async: true,
                      url: siteUrl + "/api/deleteOutcomeAchievement/" + id,
                      headers: {
                        'Accept': 'application/json',
                        'Authorization': 'Bearer ' + localStorage.token
                      },
