	...
	$.ajax({async: true,
		...
		success: function(result){
			...
			$.each( result.schemes, function( key, value ) {
			   ...
			   var htm = '<div class="col-md-4 dash-sec-thumb-main" data-perf="'+perf+'"><div class="dash-sec-thumb '+cls+'"><a class="anch" data-target="#chartDrop2_'+key+'"><div class="title">'+sector.name+'</div><div class="thumb-ind-wrap"><span class="thumb-ind-item">'+result.totalEst[key]+'</span><span class="thumb-ind-item green">'+result.totalExp[key]+'</span></div></a><div class="sub-anchor-drop"><div class="sub-anchor-drop-in"><a href="scheme_dashboard.html?scheme_id='+sector.id+'">View Scheme Details</a></div></div><div class="dash-sec-thumb-dets" id="chartDrop2_'+key+'"><canvas id="chartCan2_'+key+'" width="500"></canvas></div></div></div>';
