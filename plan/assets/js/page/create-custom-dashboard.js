...
$('#addToDashb').on('click', function(){
	...
	$('#customDashboardCreate').on('submit', function(e){
		$.notify('Creating Custom Dashboard', 'info');
		//alert('here');
		e.preventDefault();
        //if($.validatr.validateForm($('#scheme-objective-output-add'))){
			var form_data = $(this).serialize();
			$.ajax({async: true,
				url: siteUrl + "/api/createCustomDashboard",
				headers: {
					'Accept': 'application/json',
					'Authorization': 'Bearer ' + localStorage.token
				},
				...
				success: function(result){
					$('.notifyjs-wrapper').trigger('notify-hide');
					$.notify('Custom Dashboard Created', 'success');
					console.log(result);
					//location.reload();
