                ...
                foreach ($value6->output_inds as $key7 => $value7) {
                    ...
                    if((!empty($value7->outcome_inds)) && $outc_count > 0 ){
                        ...
                        foreach ($value7->outcome_inds as $key8 => $value8) {
                            ...
                            if($j==0){
                                $row_arr[$i]['scheme'] = $value5->name;
                                $row_arr[$i]['financials'] = "<b>Total Allocation:</b><br>Rs ".(($value5->financials['total_est'])/100)." Cr,<br><b>Revenue:</b>".(($value5->financials['revenue'])/100)."Cr., <b>Capital</b>".(($value5->financials['capital'])/100)."Cr., <b>Loan:</b>".(($value5->financials['loan'])/100)."Cr.<br><b>Total Expenditure:</b><br>Rs ".(($value5->financials['total_exp'])/100)." Cr";
                                $row_arr[$i]['sl'] = $sl;
                                $sl++;
                            }
                            else{
                                $row_arr[$i]['scheme'] = "&nbsp;";
                                $row_arr[$i]['financials'] = "&nbsp;";
                                $row_arr[$i]['sl'] = "&nbsp;";
                            }
                            if($k==0){
                                $row_arr[$i]['objective'] = $value6->description;
                        ...
                        if($j==0){
                            $row_arr[$i]['scheme'] = $value5->name;
                                $row_arr[$i]['financials'] = "<b>Total Allocation:</b><br>Rs ".(($value5->financials['total_est'])/100)." Cr, <br><b>Total Expenditure:</b><br>Rs ".(($value5->financials['total_exp'])/100)." Cr";
                            $row_arr[$i]['sl'] = $sl;
                            $sl++;
                        }
                        else{
