$(document).ready(function(){
	...
	$.ajax({async: true,
		url: siteUrl + "/api/outcome_reviews_head_info",
		headers: {
	...
	$.ajax({async: true,
		url: siteUrl + "/api/outcome_reviews_list",
		headers: {
			'Accept': 'application/json',
			'Authorization': 'Bearer ' + localStorage.token
		},
	...
	$('#review-add').on('submit', function(e){
        ...
        if($.validatr.validateForm($('#review-add'))){
			...
			$.ajax({async: true,
				url: siteUrl + "/api/outcome_reviews_add",
				headers: {
					'Accept': 'application/json',
					'Authorization': 'Bearer ' + localStorage.token
				},
	...
	$(document).on('click', '.reviewedit', function(){
 	...
 	$('#editReviewForm').on('submit', function(e){
        ...
        if($.validatr.validateForm($('#editReviewForm'))){
			var form_data = $(this).serialize() + '&achievement_id=' + achievement_id;
			$.ajax({async: true,
				url: siteUrl + "/api/outcome_reviews_edit",
				headers: {
					'Accept': 'application/json',
					'Authorization': 'Bearer ' + localStorage.token
				},
	...
	$(document).on('click', '.deleteOutcomeReview', function(){
	var id = $(this).attr('data-id');
	if(confirm("Are you sure to delete?")){
		
		$.ajax({async: true,
                      url: siteUrl + "/api/deleteOutcomeReview/" + id,
                      headers: {
                        'Accept': 'application/json',
                        'Authorization': 'Bearer ' + localStorage.token
                      },
