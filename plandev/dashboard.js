...
$.ajax({async: true,
	...
	success: function(result){
		...
		var data = [
			...
			{ label: 'Total Expenditure(Rs in Crores)', y: result.totalExp, color: '#37942c' },
		...
		$.each( result.departments, function( key, value ) {
		   ...
		   if (isFinite(perf))
			...
			if(result.totalExp[key]){
				...
				var data = [
					{ label: 'Estimate', y: result.totalEst[key], color: "#37942c" },
	...
	success: function(result){
		...
		data = {
		    datasets: [{
		        ...
		        backgroundColor:['#37942c', '#b70000', '#FFFF00','#999']
		    }],
		    labels: [
		...
		$.each( result.sectors, function( key, value ) {
		   ...
		   if (!isFinite(perf))
			...
			if(total_ind){
				...
				// };
				var data = [
					{ label: "On track", y: result.xontrack[sector.id], color: "#37942c" },
...
function drawBarChartStacked(target, barChartData, yGap){
	...
	// var barchart_data_dec = JSON.parse(barChartData);
